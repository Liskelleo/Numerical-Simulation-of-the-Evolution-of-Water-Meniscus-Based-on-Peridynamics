% 本脚本用于计算单液桥简化模型算例的弯液面动态演变，采用键型近场动力学方法
% 作者: Liskelleo
clc; clear all;
% 加载数据
data = load('soil_board.dat');
indices = data(:, 1); % 第一列是索引
x_0 = data(:, 2); % 第二列是x坐标
y_0 = data(:, 3); % 第三列是y坐标
labels = data(:, 4); % 第四列是标签

data1 = load('air_backround.dat');
x1 = data1(:, 2);
y1 = data1(:, 3);
...
v_wall = [0, 0]; % 固体墙速度矢量
a_wall = [0, 0]; % 固体墙加速度矢量
v_fmax = ; % 最大流体速度，一般取为1；若对于溃坝问题，等于sqrt(gH)，g为重力加速度
c_f =; % 流体声速，与最大流体速度有关，一般取10 * v_fmax
gamma_f = ; % 流体压强常数
density_f0 = ; % 初始流体密度
reference_pressure = density_f0 * (c_f)^2 / gamma_f; % 参考压强
theta_eq = ; % 固液静态接触角theta_eq，示例值为5 * pi / 180
beta = ; % 表面张力系数
miu = ; % 动力粘度系数
...
hhh = zeros(length(x_0), 1);% 用于储存表面张力的大小（y轴分量）
displacement = zeros(length(x_0), 2);
...
% break
%这个break是用来画图的，step==2时break用来画初始图象
...
%%% 以下代码是筛出上下自由液面表层的粒子并分类的（应该是？好久没运行这一块代码了）
%             if step ==1
%                 % 初始化空数组
%                 xA = [];yA = [];
%                 xB = [];yB = [];
...
%%% 以下代码是筛出上下自由液面表层的粒子并分类，并且每个筛出的粒子具有一个物理属性
%             if step == 150
%                 % 初始化空数组
...
%                 vfrac_s = 0.5150;
                ...
                hhh(original_index, :) = surface_force(:,2);% norm(surface_force);
            end
%             disp(body_force(364,:));
% if step ==50
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
        ...
        fidnew=fopen('soil_board_now.dat','w+'); % 打开文件用于写入数据
        ...
        for i = 1:length(indices)
            ...
            fprintf(fidnew, '%15.5e\t', velocity_magnitude(i));      % 写入每个粒子的密度density或其他物理属性如velocity_magnitude/hhh...
            fprintf(fidnew, '\n');                        % 换行
        end

        fclose(fidnew); % 关闭文件
        
% % %  以下代码用于输出每个时间步的图象（也可以设置间隔，此处未设置）
% %         figure;
...
%         filename = sprintf('frame_%04d.png', step); % 保存为PNG文件
...
% 展示结果
figure;
final_data = load('soil_board_now.dat');
...
scatter(x_new(labels == 1), y_new(labels == 1), 20, density_new(labels == 1), 'filled'); hold on;
% scatter(xA_notInB, yA_notInB);
c = colorbar; % 添加颜色条以显示密度值
c.Label.String = '速度 (×10^-^3 cm/s)';
xlabel('x');
ylabel('y');
axis equal;
xlim([min(filtered_points_x) - 0.1, max(filtered_points_x) + 0.1]);
